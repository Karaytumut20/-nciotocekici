...
test("ana sayfada tek H1, canonical ve yatay taşma yoktur", async ({ page }) => {
  ...
  await expect(page.locator('link[rel="canonical"]')).toHaveAttribute("href", "https://inciotocekici.com");
...
test("sitemap içindeki bütün URL’ler 200 döner", async ({ request }) => {
  ...
  for (const canonicalUrl of urls) {
    const localUrl = canonicalUrl.replace("https://inciotocekici.com", "http://localhost:3000");
